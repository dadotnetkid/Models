...
namespace Services
{
    public partial class SmsServices
    {
        ...
        {
            RestClient restClient = new RestClient("http://sms-server.jp.ngrok.io/");
            RestRequest restRequest = new RestRequest("api/sms/send", Method.POST);
            restRequest.AddParameter("username", "bms");
            restRequest.AddParameter("password", "123321");
            restRequest.AddParameter("apicode", "ABCD-12345");
            restRequest.AddParameter("PhoneNumber", number);
            restRequest.AddParameter("Message", message);
            var resp = restClient.Execute<SmsResult>(restRequest);
            return resp.Data;
        }
    }
